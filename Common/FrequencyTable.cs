 ...
 * 
 * 
 * Modified by Steve (Ambo) to support long as frequency type rather than int.
 * Also added methods to support adding a specified quantity of a value as well as some other changes.
 */
using System;
...
namespace EveMarketMonitorApp.Common
{
	...
	public struct CumulativeFrequencyTableEntry<T>
	{
        ...
        public CumulativeFrequencyTableEntry(T value, double CumRelativeFrequency, long CumAbsoluteFrequency)
		{
			Value = value;
		...
		/// </summary>
        public long CumulativeAbsoluteFrequency;
	}
	/// <summary>
	...
	public struct FrequencyTableEntry<T> where T : IComparable<T>
	{
		...
		public FrequencyTableEntry(T val, long absFreq, double relFreq, double percentage)
		{
			Value = val;
	...
	{
		#region privates
        //Dictionary<T, List<long>> _positions;
        ...
        private Dictionary<T, long> _entries;
		...
		{
            //_positions = new Dictionary<T, List<long>>();
            _entries = new Dictionary<T, long>();
		...
		{
            //_positions = new Dictionary<T, List<long>>();
            _entries = new Dictionary<T, long>(initialCapacity);
			_relFrequencies = new Hashtable();
        ...
        // Added by Steve (Ambo)
        public void Add(T value, long quantity)
        {
            //List<long> _tempPos;
            if (_entries.ContainsKey(value))
            {
                // update the frequency
                _entries[value]+=quantity;
                // update mode and highest frequency
                if (_entries[value] > _high)
                {
                    _high = _entries[value];
                ...
                // add quantity to sample size
                _count+=quantity;
                UpdateRelFrequencies();
                UpdateSumAndMean(value, quantity);
                // store the actual position of the entry in the dataset
                //_positions.TryGetValue(value, out _tempPos);
                //long[] positions = new long[quantity];
                //positions[0] =  _count - quantity + 1;
                //for (long i = 1; i < quantity; i++)
                //{
                //    positions[i] = positions[i - 1] + 1;
                //}
                //_tempPos.AddRange(positions);
                // remove old entry
                //_positions.Remove(value);
                // store new entry
                //_positions.Add(value, _tempPos);
            ...
            else
            {
                ...
                UpdateRelFrequencies();
                UpdateSumAndMean(value, quantity);
                // create a new entry and set positions
                //_tempPos = new List<long>();
                //long[] positions = new long[quantity];
                //positions[0] = _count - quantity + 1;
                //for (long i = 1; i < quantity; i++)
		...
		public void Remove(T value)
		{
			if (_entries.ContainsKey(value))
			{
                ...
                _length --;
                // Remove the entry
                if (_high == _entries[value]) { UpdateHighFrequencyAndMode(); }
                UpdateSumAndMean(value, _entries[value]);
        ...
        /// </summary>
        /// <param name="value">Value to remove</param>
        public void Remove(T value, long quantity)
        {
            if (_entries.ContainsKey(value))
            {
                if (_entries[value] < quantity)
                {
                    quantity = _entries[value];
                }
                // Update length and sample size
                _count -= quantity;
                if (_high == _entries[value]) { UpdateHighFrequencyAndMode(); }
                if (_entries[value] - quantity == 0)
                {
                    // Remove the entry
                    _entries.Remove(value);
                    //_positions.Remove(value);
                ...
                UpdateRelFrequencies();
                UpdateSumAndMean(value, quantity * -1);
            }
            else
                throw new InvalidOperationException();
        }
        ...
        /// </summary>
        /// <param name="value">Value</param>
        private void UpdateSumAndMean(T value, long quantity)
        {
            // error handling is a bit circuitous (but safe IMHO)
            if ((value is string) || (value is char))
            {
            ...
            else
            {
                ...
                try
                {
                    _dblSum += ((double)Convert.ChangeType(value, TypeCode.Double) * quantity);
                    _mean = _dblSum / (double)SampleSize;
                }
                catch
                {
		...
		public IEnumerator<FrequencyTableEntry<T>> GetEnumerator()
		{
			...
			foreach (T key in _entries.Keys)
			{
                long_f = _entries[key];
				// fill the structure
                _output = new FrequencyTableEntry<T>(key, long_f, (double)_relFrequencies[key], (double)_relFrequencies[key] * 100.0);
		...
		public T[] GetData(bool Pristine)
		{
            ...
            if (SampleSize != 0)
            {
                ...
                else
                {
                    ...
                    //}
                    long counter = 0;
                    Dictionary<T, long>.Enumerator enumerator = _entries.GetEnumerator();
                    while(enumerator.MoveNext()) 
                    {
                        T value = enumerator.Current.Key;
                        for (int i = 0; i < enumerator.Current.Value; i++)
                        {
        ...
        // Added by Steve (Ambo)
        /// <summary>
        /// Update highest frequecy and mode from data in _entries
        /// </summary>
        private void UpdateHighFrequencyAndMode()
        {
            _high = 0;
            Dictionary<T,long>.Enumerator enumerator = _entries.GetEnumerator();
            while (enumerator.MoveNext())
            {
                if (enumerator.Current.Value > _high) 
                { 
                    _high = enumerator.Current.Value;
                    _mode = enumerator.Current.Key;
                }
            }
        }
        ...
        // Added by Steve (Ambo)
        /// <summary>
        /// Update relative frequencies table (note: _count and _entries MUST be accurate first)
        /// </summary>
        private void UpdateRelFrequencies()
        {
            _relFrequencies = new Hashtable();

            foreach (T key in _entries.Keys)
            {
                _relFrequencies.Add(key, (double)_entries[key] / _count);
            }
        ...
        // ---------------------------------------------------------------------------------
        // Added by Steve (Ambo)
        // Was going to use this but then figured out that the 'positions' structure
        // Is not really required.
        // ---------------------------------------------------------------------------------
    
        /// <summary>
        /// Adjust the '_positions' data structure be removing the specified quantity of the specified
        /// value.
        /// subsequent values are then shifted 'backwards' to fill the spaces that are created.
        /// </summary>
        /// <param name="valueToRemove"></param>
        /// <param name="quantityToRemove"></param>
        /*
        private void AdjustPositions(T valueToRemove, long quantityToRemove)
        {
            Dictionary<long, long> change = new Dictionary<long, long>();
            int removed = 0;
            List<int> positions = _positions[valueToRemove];
		...
		private class SortFrequencyTable<K> where K : IComparable<K>
		{
            ...
            private static void SortByValueAscending(long l, long u)
			{
                if (u >= 0)
                {
                    long i = l;
			...
			/// <param name="l">lower bound</param>
			/// <param name="u">upper bound</param>
            private static void SortByValueDescending(long l, long u)
			{
                if (u >= 0)
                {
                    long i = l;
			...
			/// <param name="l">lower bound</param>
			/// <param name="u">upper bound</param>
            private static void SortByFrequencyAscending(long l, long u)
			{
                if (u >= 0)
                {
                    long i = l;
                    long j = u;
                    long v = a[(l + u) / 2].AbsoluteFreq;
                    while (i <= j)
                    {
			...
			/// <param name="l">lower bound</param>
			/// <param name="u">upper bound</param>
            private static void SortByFrequencyDescending(long l, long u)
			{
                if (u >= 0)
                {
                    long i = l;
